...
\begin{frame}{HyperLogLog++ algorithm}
  ...
  % trong đó chỉ số 0 trong các mảng tương ứng với các giá trị độ chính xác 4. 
  % Như một ví dụ, đối với $m = 2^{10} and p = 10$ 
...
\begin{frame}{HyperLogLog++ algorithm}
  ...
  \end{itemize}
  % Theo các thí nghiệm thực hiện bởi các tác giả của HyperLogLog++, ước lượng $n_{lin}$ 
  % được tính theo thuật toán 
  % Linear Counting vẫn tốt hơn cho các số lượng phần tử nhỏ so với giá trị được hiệu 
  % chỉnh sai số $n$. 
  % Do đó, nếu ít nhất một bộ đếm trống tồn tại, 
  ...
  % Trong trường hợp như vậy, giá trị được hiệu chỉnh sai số $n$ chỉ được sử dụng khi ước lượng tuyến tính $n_{lin}$ 

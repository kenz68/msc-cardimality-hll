...
%Redis sử dụng một triển khai của HyperLogLog gọi là "sparse representation" cho các bộ đếm nhỏ và "dense representation" cho các bộ đếm lớn hơn. Dung lượng bộ nhớ của HyperLogLog phụ thuộc vào số lượng thanh ghi (registers), và mỗi thanh ghi lưu trữ thông tin về vị trí của bit đầu tiên là 1 trong chuỗi băm.\\

\textbf{Số lượng thanh ghi (m)}:
\begin{itemize}
	\item $m = 2^p$, trong đó p là số bit để xác định số thanh ghi.
	\item Giá trị mặc định của p trong Redis là 14, nghĩa là có $2^{14} = 16384$ thanh ghi.
\end{itemize}
\textbf{Dung lượng bộ nhớ cần thiết}
\begin{itemize}
	\item Mỗi thanh ghi cần 6 bit để lưu trữ vị trí của bit đầu tiên là 1.
	\item Tổng dung lượng bộ nhớ cần thiết cho HyperLogLog trong Redis có thể tính theo công thức: 
%	\[Memory=mx6 bits\], với m = 16384 thanh ghi:
%	\[Memory=16384×6 bits=98304 bits=12288 bytes=12 KB\]
\end{itemize}
\end{frame}
